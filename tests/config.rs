...
#[test]
fn external_acp_gateway_disables_local_sidecar_launch() {
    let dir = tempdir().unwrap();
    ...
    let config = Config::load_with_env(dir.path(), &env).unwrap();
    assert_eq!(config.workbuddy_acp_url, "http://host.docker.internal:44741");
    assert_eq!(config.workbuddy_sidecar_mode, "external");
    assert_eq!(
        config.workbuddy_external_cwd,
        Some(std::path::PathBuf::from("C:/Users/test/workspace"))
    );
}

#[test]
fn rejects_invalid_external_gateway_configuration() {
    for (url, cwd) in [
        ("", "/workspace"),
        ("   ", "/workspace"),
        ("not-a-url", "/workspace"),
        ("ftp://gateway.example", "/workspace"),
        ("http://", "/workspace"),
        ...
        let env = HashMap::from([
            ("HOME".into(), dir.path().to_string_lossy().to_string()),
            ("WORKBUDDY_SIDECAR_MODE".into(), "external".into()),
            ("WORKBUDDY_ACP_URL".into(), url.into()),
            ("WORKBUDDY_EXTERNAL_CWD".into(), cwd.into()),
        ]);
        assert!(Config::load_with_env(dir.path(), &env).is_err(), "{url} {cwd}");
    }

    let dir = tempdir().unwrap();
    let env = HashMap::from([
        ("HOME".into(), dir.path().to_string_lossy().to_string()),
        ("WORKBUDDY_SIDECAR_MODE".into(), "invalid".into()),
    ]);
    assert!(Config::load_with_env(dir.path(), &env).is_err());
}

#[test]
fn direct_http_ignores_inactive_external_gateway_settings() {
    let dir = tempdir().unwrap();
    let env = HashMap::from([
        ("HOME".into(), dir.path().to_string_lossy().to_string()),
        (
        ...
        ("WORKBUDDY_SIDECAR_MODE".into(), "external".into()),
        ("WORKBUDDY_ACP_URL".into(), "".into()),
    ]);
    let config = Config::load_with_env(dir.path(), &env).unwrap();
    assert_eq!(config.transport, "http");
    assert_eq!(config.workbuddy_external_cwd, None);
}
