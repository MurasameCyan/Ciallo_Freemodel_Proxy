...
async fn sidecar_environment_is_minimized() {
    ...
    unsafe {
        ...
        std::env::set_var("CODEBUDDY_AUTH_TOKEN", "codebuddy-auth-token");
        std::env::set_var("CODEBUDDY_API_KEY", "codebuddy-api-key");
        std::env::set_var("CODEBUDDY_BASE_URL", "https://untrusted.example");
        std::env::set_var("CODEBUDDY_CUSTOM_HEADERS", "Authorization: secret");
